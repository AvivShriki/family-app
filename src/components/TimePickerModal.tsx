...
export default function TimePickerModal({ visible, value, onSelect, onClose }: Props) {
  ...
  const confirm = () => { onSelect(`${hour}:${minute}`); onClose(); };
      ...
      <View style={styles.overlay}>
        <View style={styles.sheet}>
              ...
              <ScrollView style={styles.colScroll}>
                {HOURS.map((hh) => (
                  <TouchableOpacity
                    ...
                    onPress={() => setHour(hh)}
                  >
                    <Text style={[styles.optionText, hour === hh && styles.optionTextActive]}>{hh}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
            ...
            <View style={styles.column}>
              ...
              <ScrollView style={styles.colScroll}>
                {MINUTES.map((mm) => (
                  ...
                  >
                    <Text style={[styles.optionText, minute === mm && styles.optionTextActive]}>{mm}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
          ...
          <TouchableOpacity style={styles.confirmBtn} onPress={confirm}>
            <Text style={styles.confirmText}>אישור {hour}:{minute}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  columns: { flexDirection: 'row', justifyContent: 'center', alignItems: 'flex-start', gap: spacing.lg, marginBottom: spacing.lg },
  column: { alignItems: 'center' },
  colLabel: { fontSize: 12, color: colors.textLight, marginBottom: spacing.xs },
  colScroll: { height: 180, width: 70 },
  option: { paddingVertical: spacing.sm, alignItems: 'center', borderRadius: radius.md, marginBottom: 4 },
  ...
  confirmBtn: { backgroundColor: colors.pinkAccent, borderRadius: radius.full, padding: spacing.md, alignItems: 'center' },
